...
def generate_tusimple(input_dir, val_size):
    ...
    return out
def generate_iscwebots(input_dir, val_size):
    # Read from train.txt and val.txt
    out = {}
    ...
    for split in ['train', 'val']:
        ...
        img_paths = load_list(split_file)
        split_imgs = {}
        for img_path in img_paths:
            img_id = os.path.splitext(os.path.basename(img_path))[0]
            img_info = {
                "raw_file": img_path,
                "lanes": [],       # Optional: fill in if you want
                "h_samples": [],   # Optional: used for TuSimple evaluation
                "pts": None        # Optional: can be added later
            }
            split_imgs[img_id] = img_info
        out[split] = split_imgs
...
if __name__ == '__main__':
    ...
    elif args.dataset == 'iscwebots':
        out = generate_iscwebots(args.input_dir, args.val_size)
    else:
